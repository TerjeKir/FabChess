...
impl CacheEntry {
    ...
    pub fn u16_to_mv(mv: u16, game_state: &GameState) -> GameMove {
        ...
        let from_board = BitBoard(1u64 << from);
        let to = ((mv & 0x03F0) >> 4) as u8;
        let to_board = BitBoard(1u64 << to);
        ...
        for pt in PIECE_TYPES.iter() {
            if (game_state.get_piece(*pt, color_to_move) & from_board).not_empty() {
                piece_type = *pt;
            }
        }
        if typ == 1 {
            ...
            for pt in PIECE_TYPES.iter() {
                if (game_state.get_piece(*pt, enemy_color) & to_board).not_empty() {
